...
func main() {
	...
	serviceName := constants.ServiceAuth
	...
	// 验证必需的配置
	if err := envCfg.ValidateRequired(serviceName, []string{"DATABASE_DSN", "JWT_SECRET"}); err != nil {
		panic(err)
	}
	...
	slog.Info("Auth server is starting", "host", c.Host, "port", c.Port)
	server.Start()
}
